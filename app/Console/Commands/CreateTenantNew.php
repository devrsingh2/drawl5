...
class CreateTenantNew extends Command
{
     ...
     * @var string
     */
    protected $signature = 'tenantnewmodification:create';
    ...
    public function handle()
    {
        ...
        $username = $this->value('administrator username');
        $email = $this->value('administrator email');

        if (!$this->confirmData($fqdn, $redirect, $https, $maintenance, $name, $username, $email)) {
...
//        Tenant::registerAdmin($name, $username, $adminPassword, $email)->notify(new TenantCreated($subdomain));
        ...
        $this->info("Administrator {$email} / {$username} can sign in, using password: {$adminPassword}");
     ...
     * @param $email
     */
    private function confirmData($fqdn, $redirect, $https, $maintenance, $name, $username, $email)
    {
